...
export default function UsersPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Step 2: Sinkronisasi ke Group jika ada yang dipilih
  ...
  // ── Action: Revise / Edit ──────────────────────────────
  const handleRevise = (userId: string) => {
    // TODO: Implementasi logika edit (buka modal edit / navigasi ke page edit)
    console.log('Revise user:', userId);
    alert(`Fitur Revisi untuk User ID: ${userId} akan segera diimplementasikan.`);
  };
          ...
          <tbody className="divide-y divide-slate-800">
            {paginatedUsers.length === 0 ? (
              ...
              paginatedUsers.map((user) => {
                // Map relasi Group & Talkgroup untuk ditampilkan sebagai badges
                  ...
                  <tr key={user.id} className="hover:bg-slate-800/50 transition-colors">
                    ...
                    <td className="px-4 py-3 align-top pt-4">
                      <span className={`px-2 py-1 text-[10px] uppercase font-bold rounded tracking-wider ${
                        user.role === 'admin' ? 'bg-red-900/50 text-red-400 border border-red-800/50' : 'bg-slate-800 text-slate-300 border border-slate-700'
                      }`}>
                        {user.role}
                      </span>
                    </td>
                    ...
                    <td className="px-4 py-3 align-top">
                      {assignedGroups.length === 0 ? (
                        ...
                        <div className="flex flex-wrap gap-1.5 items-center relative">
                          {/* Tampilkan maksimal 3 badge pertama */}
                    ...
                    <td className="px-4 py-3 text-center align-top pt-3">
                      <button
                        onClick={() => handleRevise(user.id)}
                        className="bg-slate-700 hover:bg-slate-600 text-slate-200 text-xs px-3 py-1.5 rounded transition-colors border border-slate-600"
                      >
                        REVISI
                      </button>
                    </td>
      ...
      <Modal isOpen={isModalOpen} onClose={resetCreateModal} title="PROVISION NEW OPERATOR">
        <form onSubmit={handleCreate} className="space-y-4">
          ...
          <div>
            ...
            <input
              type="text"
              placeholder="Ketik nama group (misal: HRD)..."
